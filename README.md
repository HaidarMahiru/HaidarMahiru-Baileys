...
1. ``` cd path/to/HaidarMahiru-Baileys ```
2. ``` yarn ```
3. ``` yarn example ```
...
```ts
   ...
   const buttonMessage = {
     ...
     ephemeralExpiration: 864000,
     buttons: buttons,
     viewOnce: true,
     headerType: 1
                }
    sock.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: sock.parseMention("" + ""), forwardingScore: 999999, isForwarded: true }})
```
